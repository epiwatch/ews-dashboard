...
import { Chart as ChartJS, ArcElement, Tooltip, CategoryScale, LinearScale } from "chart.js";
...
import { LegendColours, DateFormat, FetchDataDateFormat } from "./constant";
import BarChart from "./components/BarChart";
import PieChart from "./components/PieChart";
import BarChartLegend from "./components/BarChartLegend";
import { transformData, transferColorProperty } from "./utils/dataUtils";

import { Country, Disease, Syndrome, DiseaseInLocations, PieChartDatasetsItem, BarChartDatasetsItem, BarChartLegendItem } from "@/types";
...
export default function Home({ countries, diseases, syndromes, subregions }: InferGetServerSidePropsType<typeof getServerSideProps>) {

  const defaultCountry = countries.filter((countryData: Country) => countryData.country_name === "Australia")[0];
  ...
  const [countryTitleText, setCountryTitleText] = useState<string>("Distribution within Australia");
  const [regionTitleText, setRegionTitleText] = useState<string>("Distribution within Australia and New Zealand");
  const [stackedBarChartTitle, setStackedBarChartTitle] = useState<string>("Reports for Australia");
  const [autoTop10, setAutoTop10] = useState<boolean>(true);

  const [stackedBarChartLegend, setStackedBarChartLegend] = useState<Array<BarChartLegendItem>>([]);
  const [stackedBarChartData, setStackedBarChartData] = useState<BarChartDatasetsItem[]>([]);
  const [stackedBarChartLabels, setStackedBarChartLabels] = useState<Array<string>>([]);

  const [countryPieChartLabels, setCountryPieChartLabels] = useState<Array<string>>([]);
  const [countryPieChartData, setCountryPieChartData] = useState<PieChartDatasetsItem[]>([]);

  const [countryRegionChartLabels, setRegionPieChartLabels] = useState<Array<string>>([]);
  const [countryRegionChartData, setRegionPieChartData] = useState<PieChartDatasetsItem[]>([]);
  ...
  const checkTop10Diseases = (diseaseData: DiseaseInLocations) => {
    let newSelectedDiseases = [] as Disease[];
    let diseaseArray: { name: string, count: number }[] = [];
    ...
    diseaseArray.map((item) => newSelectedDiseases.push(diseases.filter((d: Disease) => d.disease == item.name)[0]));
    setSelectedDiseases(newSelectedDiseases);

    // add the selected diseases and selected syndromes into 1 list
    const appliedSelectionWithColor = newSelectedDiseases.map((item: any, idx) => {
  ...
  const createStackedChart = async () => {
    ...
    await fetch(process.env.NEXT_PUBLIC_API_URL + "/reportDiseasesInRange", { ...postMethod, body: JSON.stringify(rangePayload) }).then((res) => {
      if (res.status === 200) {
        return res.json();
      }
    }).then((response) => {
      ...
      const colorData = transferColorProperty(transformedData, appliedSelections);
  ...
  const getRegionalPieChartData = async () => {
    ...
    await fetch(process.env.NEXT_PUBLIC_API_URL + "/reportIllnessesInRegion", { ...postMethod, body: JSON.stringify(regionPayload) }).then((res) => {
      if (res.status === 200) {
        return res.json();
      }
    }).then((response) => {
      ...
      const colorData = transferColorProperty(formattedData, appliedSelections);

      setRegionPieChartLabels(formattedData.map((item: any) => item.label));

      const data = formattedData.map((item: any) => item.count);
      const backgroundColor = colorData.map((item: any) => item.backgroundColor);
  ...
  const getCountryPieChartData = async () => {
    ...
    await fetch(process.env.NEXT_PUBLIC_API_URL + "/reportDiseasesInLocation", { ...postMethod, body: JSON.stringify(countryPayload) }).then((res) => {
      if (res.status === 200) {
        return res.json();
      }
    }).then((response) => {
  ...
  const updateCharts = async () => {
    ...
    for (const [key, value] of Object.entries(subregions) as [string, string][]) {
  ...
  return (
    <Grid container spacing={2}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid xs={12} md={4}>
                <Autocomplete
                  ...
                  size="small"
                  options={countries}
                  getOptionLabel={(option: Country) => (`${option.country_name} (${option.iso3})`)}
                  includeInputInList
                  autoComplete
                  disableClearable
                  value={country}
              ...
              <Grid xs={24} md={24}>
                <div className={styles.applyButton}>
                  <FormGroup>
                    <FormControlLabel control={<Switch checked={autoTop10} onChange={handleSwitchChange} />} label="Auto Disease Top 10" />
                  </FormGroup>
                  {/* TBD: add disable logic - only when filter values changes */}
                  <Button variant="contained" color="success" onClick={updateCharts}>Apply</Button>
                </div>
              </Grid>
      ...
      <BarChart labels={stackedBarChartLabels} datasets={stackedBarChartData} title={stackedBarChartTitle} />

      <PieChart labels={countryPieChartLabels} datasets={countryPieChartData} title={"Country distribution"} subTitle={countryTitleText} />
      <PieChart labels={countryRegionChartLabels} datasets={countryRegionChartData} title={"Regional distribution"} subTitle={regionTitleText} />
    </Grid >
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const countries = await (await fetch(process.env.NEXT_PUBLIC_API_URL + "/countries")).json();
  const diseases = await (await fetch(process.env.NEXT_PUBLIC_API_URL + "/disease")).json();
  const syndromes = await (await fetch(process.env.NEXT_PUBLIC_API_URL + "/syndrome")).json();
  const subregions = await (await fetch(process.env.NEXT_PUBLIC_API_URL + "/subregions")).json();
